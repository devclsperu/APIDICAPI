...
const recordController = new record_controller_1.RecordController();
// Middleware para validar el parámetro de horas
const validateHoursParam = (req, res, next) => {
    const hours = parseInt(req.params.hours);
    if (isNaN(hours) || hours < 2 || hours > 24) {
        logger_1.logger.warn(`Parámetro de horas inválido: ${req.params.hours}`);
        res.status(400).json({
            success: false,
...
const validateDateRange = (req, res, next) => {
    ...
    if (!date) {
        logger_1.logger.warn('Falta parámetro de fecha');
        res.status(400).json({
            success: false,
    ...
    if (!dateRegex.test(date)) {
        logger_1.logger.warn(`Formato de fecha inválido: date=${date}`);
        res.status(400).json({
            success: false,
    ...
    if (isNaN(selectedDate.getTime())) {
        logger_1.logger.warn(`Fecha inválida: ${date}`);
        res.status(400).json({
            success: false,
...
router.get('/last-hour', auth_middleware_1.validateToken, rateLimit_config_1.lastHourLimiter, validateLastHourRoute, (req, res, next) => {
    // Verificación adicional para asegurar que no haya parámetros adicionales
    if (Object.keys(req.query).length > 0) {
        logger_1.logger.warn(`Parámetros no permitidos en la ruta last-hour: ${JSON.stringify(req.query)}`);
        res.status(400).json({
            success: false,
...
router.get('/last/:hours', auth_middleware_1.validateToken, rateLimit_config_1.lastHoursLimiter, validateHoursParam, recordController.getLastHoursRecords.bind(recordController));
// Nueva ruta para consulta por rango de fechas (DEBE IR ANTES QUE /:id)
router.get('/date-range', auth_middleware_1.validateToken, rateLimit_config_1.dateRangeLimiter, validateDateRange, recordController.getRecordsByDateRange.bind(recordController));
// Nueva ruta para obtener todas las transmisiones del día actual (DEBE IR ANTES QUE /:id)
router.get('/all-day', auth_middleware_1.validateToken, rateLimit_config_1.allDayLimiter, recordController.getAllDayRecords.bind(recordController));
// Ruta para obtener registros por ID específico (DEBE IR AL FINAL)
router.get('/:id', auth_middleware_1.validateToken, rateLimit_config_1.recordsByIdLimiter, (req, res, next) => {
    // Verificar que el ID no sea 'last-hour' o alguna variación
    if (req.params.id.toLowerCase().includes('last-hour')) {
        logger_1.logger.warn(`Intento de acceso a last-hour con variación: ${req.params.id}`);
        res.status(404).json({
            success: false,
            error: 'Endpoint no encontrado',
            details: {
